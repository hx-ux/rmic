...
fn command_params() {
    let name = "water_params";
    let effect = |gmic: Gmic| gmic.add_filter("water", &["100", "1", "45"]);
    let result = process_images(name, effect);
    assert!(result.is_ok());
}
...
fn commands_stacked() {
    let name = "card";
    let effect = |gmic: Gmic| {
        gmic.add_filter("light_patch", &["500", "0.9", "1.7"])
            .add_filter("glow", &["10%"])
            .add_raw_filter("polaroid 5,30")
    };
    let result = process_images(name, effect);
    assert!(result.is_ok());
}
...
fn raw() {
    ...
    let effect =
        |gmic: Gmic| gmic.add_raw_filter("polaroid 5,30 rotate 20 drop_shadow , drgba glow 10%");
    let result = process_images(name, effect);
...
fn raw_stacked() {
    ...
    let effect = |gmic: Gmic| {
        gmic.add_raw_filter("polaroid 5,30")
            .add_raw_filter("rotate 20")
            .add_raw_filter("drop_shadow ,")
            .add_raw_filter("drgba glow 10%")
    };
    let result = process_images(name, effect);
    assert!(result.is_ok());
}
...
fn random() {
    let name = "blur_rotate_solatize";
    let effect_chain = |g: Gmic| g.add_raw_filter("random_pattern 256");
    let result = process_images(name, effect_chain);
    assert!(result.is_ok());
}
